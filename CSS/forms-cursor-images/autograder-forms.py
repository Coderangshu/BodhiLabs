...
def main():
    ...
    expected_path = os.path.join(base_dir, "jsons", "forms.json")
    student_css_path = os.path.join(os.path.dirname(base_dir), "labDirectory", "forms", "style.css")
    ...
    for selector, props in expected.items():
        for prop, expected_val in props.items():
            if prop in student_styles.get(selector, {}):  # only check if implemented
                student_val = student_styles[selector][prop]
                if student_val == expected_val.lower():
                    results.append({
